...
class TimelineViewModelFactory(
    ...
    private val isAutoLoadEnabled: Boolean = true
    ...
    override fun <T : ViewModel?> create(modelClass: Class<T>): T {
        if(modelClass == TimelineViewModel::class.java){
            ...
            val timelineCapture = if(isAutoLoadEnabled) miApplication.timelineCapture else null

            val misskeyAPI = miApplication.misskeyAPIService ?: MisskeyAPIServiceBuilder.build(connectionInstance.instanceBaseUrl)

            return TimelineViewModel(connectionInstance, requestSetting, misskeyAPI ,noteCapture, timelineCapture) as T
